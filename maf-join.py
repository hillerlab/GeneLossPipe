...
## (c) Peter Schwede, 2017: Support for arbitrary numbers of non-/concatenated
#                           input files.
#
#   ,--(else, append line to lines) -->[ Start ]<------,
#   `-----------------------------------'  |           |
#   ,--------------------------------------'           |
#   `('##maf', append lines to input; trunkate lines)--'
##
...
for i in args:
    with open(i, "r") as f:
        ...
        for lineno, line in enumerate(f):
            if lineno > 0 and line[:5] == "##maf":
                inputs.append(peekable(sortedMafInput(lines)))
                lines = []
            lines.append(line)
        inputs.append(peekable(sortedMafInput(lines)))
